...
type Row = {
  ...
  test_case: string;
  label_gold: "hateful" | "non-hateful";
};
    ...
    Array.from({ length: n }, async () => {
      while (next < items.length) {
        ...
        out[i] = await fn(items[i]!).catch(
          (e) => (console.error(`case ${i} failed:`, e.message), null),
        );
      }
    }),
  );
...
const cases = Array.from(
  { length: Math.min(SAMPLE, all.length) },
  (_, k) => all[Math.floor((k * all.length) / SAMPLE)]!,
);
...
const byFunctionality = new Map<
  string,
  { gold: string; n: number; none: number; review: number; remove: number }
>();
for (const r of judged) {
  const row = byFunctionality.get(r.functionality) ?? {
    gold: r.label_gold,
    n: 0,
    none: 0,
    review: 0,
...
console.log(
  `False reviews:  ${falseReviews.length}/${clean.length} non-hateful (${pct(falseReviews.length, clean.length)}), mod-log noise`,
);
console.log(
  `Missed hate:    ${missed.length}/${hateful.length} hateful (${pct(missed.length, hateful.length)}), no action and no log`,
);
for (const r of falseRemovals)
  console.log(
    `  wrongly removed [${r.functionality}] ${Math.round(r.j.hateScore * 100)}%: ${r.test_case}`,
  );
...
console.table(
  [0.5, 0.6, 0.7, 0.8, 0.9, 0.95].map((t) => ({
    threshold: t,
    "non-hateful above": pct(
      clean.filter((r) => r.j.hateScore > t).length,
      clean.length,
    ),
    "hateful above": pct(
      hateful.filter((r) => r.j.hateScore > t).length,
      hateful.length,
    ),
  })),
);
...
const scamLink =
  "Free Discord Nitro for everyone!! Claim yours before it runs out: https://dlscord-nitro.example/claim";
const invite = "JOIN MY SERVER NOW discord.gg/abc123 free giveaways";
const spamCases = [
  {
    name: "brand-new account, scam link",
    expect: "high_spam",
    message: scamLink,
    created: 2 * 60 * MINUTE,
    joined: 5 * MINUTE,
  ...
  {
    name: "burst of identical invites",
    expect: "high_spam",
    message: invite,
    created: 3 * DAY,
    joined: 10 * MINUTE,
    recent: Array(6).fill(invite),
  },
  {
    name: "mass-mention link drop",
    expect: "high_spam",
    message: "@everyone @here you have to see this https://bit.ly/3xYz",
    created: DAY,
    joined: 30 * MINUTE,
    recent: [],
  },
  {
    name: "old account, one self-promo link",
    expect: "medium_spam",
    message:
      "I just released my first indie game on Steam, would love feedback! https://store.steampowered.com/app/123",
    created: 900 * DAY,
    joined: 400 * DAY,
    recent: [],
  },
  {
    name: "brand-new account, normal hello",
    expect: "no_spam",
    message:
      "hi everyone! just joined, found this server through the subreddit",
    created: 60 * MINUTE,
    joined: 2 * MINUTE,
    recent: [],
  },
  {
    name: "old account, normal chat",
    expect: "no_spam",
    message: "anyone up for ranked tonight?",
    created: 1200 * DAY,
    joined: 700 * DAY,
    recent: ["gg last night", "that last round was wild"],
  },
];
...
console.table(
  spamCases.map((s, i) => ({
    ...
    confidence:
      spamResults[i]?.spamScore == null
        ? "n/a"
        : `${Math.round(spamResults[i]!.spamScore! * 100)}%`,
