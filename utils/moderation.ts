...
export function timeoutMinutes(violation: number) {
  return violation < 3 ? 0 : Math.min(FIRST_TIMEOUT_MINUTES * 2 ** (violation - 3), MAX_TIMEOUT_MINUTES);
}
...
export async function recordViolation(member: GuildMember, reason: string) {
  ...
  if (count === 2) {
    ...
    notice = "This is your final warning: your next violation will get you timed out.";
  } else if (minutes) {
    ...
    action = timedOut ? `Timed out until ${until}` : "Timeout failed; the bot needs Moderate Members and a role above theirs";
    notice = timedOut ? `You've been timed out until ${until}. Each further violation doubles the timeout.` : "";
  }
  await member
    .send(`Your message in **${member.guild.name}** was removed because it was flagged as ${reason}. ${notice}`.trim())
