...
export async function execute(i: ChatInputCommandInteraction) {
  ...
  if (typeof range === "string") return void (await i.reply({ content: range, flags: "Ephemeral" }));
  ...
  if (!i.memberPermissions.has(PermissionFlagsBits.ManageGuild) && onCooldown(`${i.guildId}:${i.user.id}`)) {
    return void (await i.reply({ content: "You can file one report every 5 minutes.", flags: "Ephemeral" }));
  }

  await i.deferReply({ flags: "Ephemeral" });
  const ticket = await createTicket(i.guild, i.member, `report-${target.username.replace(/[^a-z0-9_-]/g, "")}`).catch(
    (e) => {
...
export async function handlePageButton(i: ButtonInteraction) {
  ...
  if (!entries) return void (await i.reply({ content: "This transcript is no longer available.", flags: "Ephemeral" }));
