...
test("timeouts double from 5m and stop at Discord's 28-day max", () => {
  expect([1, 2, 3, 4, 5, 6].map((n) => timeoutMinutes(n))).toEqual([0, 0, 5, 10, 20, 40]);
...
test("recordViolation walks the ladder: warning, final warning, then growing timeouts", async () => {
  ...
  expect(await recordViolation(member, "spam")).toBe("Final warning (violation 2)");
  expect(await recordViolation(member, "spam")).toStartWith("Timed out until <t:");
  expect(await recordViolation(member, "spam")).toEndWith("(violation 4)");

  expect(timeouts).toEqual([5 * 60_000, 10 * 60_000]);
  expect(dms[0]).toBe("Your message in **Test Server** was removed because it was flagged as spam. This is a warning.");
