...
export function parseRange(from: string, to: string | null): { start: number; end: number } | string {
  const start = parseDay(from);
  const toDay = to === null ? null : parseDay(to);
  if (!start || (to !== null && !toDay)) return "Dates must be in YYYY-MM-DD format.";
  const end = toDay ? toDay.getTime() + DAY_MS : Date.now();
  if (start.getTime() >= end) return "The start date must be before the end date.";
...
export async function fetchChannelMessages(
  ...
  for (let page = 0; page < MAX_PAGES_PER_CHANNEL; page++) {
    ...
    for (const m of batch.values()) {
      if (m.author.id === userId && m.createdTimestamp < end && m.content) found.push(m);
    }
    const newest = batch.reduce((a, m) => (m.createdTimestamp > a.createdTimestamp ? m : a));
...
export async function fetchUserMessages(guild: Guild, reporter: GuildMember, userId: string, start: number, end: number) {
  ...
  const perChannel = await Promise.all(
    channels.map((c) => fetchChannelMessages(c, userId, start, end).catch(() => [])),
  );
  return perChannel
    .flat()
...
export async function createTicket(guild: Guild, reporter: GuildMember, name: string) {
  const access = [
  ...
  const modRoles = guild.roles.cache.filter(
    (r) => r.id !== guild.id && r.permissions.has(PermissionFlagsBits.ModerateMembers),
  );
  const ticket = await guild.channels.create({
    ...
    permissionOverwrites: [
      ...
      ...modRoles.map((r) => ({ id: r.id, type: OverwriteType.Role, allow: access })),
    ],
  });
...
export type TranscriptEntry = { content: string; at: number; url: string; channelId: string };
...
export function toTranscript(messages: Message[]): TranscriptEntry[] {
  return messages.map((m) => ({ content: m.content, at: m.createdTimestamp, url: m.url, channelId: m.channelId }));
}
...
export function transcriptPage(entries: TranscriptEntry[], page: number) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `Page ${page + 1}/${pages} · ${entries.length} messages` })
    .addFields(
      entries.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE).map((e, n) => ({
        ...
        value: `<t:${Math.floor(e.at / 1000)}:f> in <#${e.channelId}> · [jump](${e.url})\n${
          e.content.length > PREVIEW_CHARS ? `${e.content.slice(0, PREVIEW_CHARS)}…` : e.content
        }`,
      })),
    );
